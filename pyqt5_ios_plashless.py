...
from os import statvfs, listdir, walk, environ, chmod
...
try:
    ...
    from .build_utils import _makedirs, _write_text, _read_text, urlretrieve, which
except:
    from builders import getMakeExecutable, getPythonExecutable, getOpenExecutable, getHgExecutable
    from build_utils import _makedirs, _write_text, _read_text, urlretrieve, which
...
def _patch_file(pth, old_str, new_str, description):
    ...
    if old_str not in content:
        ...
        return False
    _write_text(pth, content.replace(old_str, new_str))
...
def _apply_qml_sip_patch(pyqt5_dir):
    ...
    if new_content != content:
        _write_text(sip_file, new_content)
...
def _create_pdy_template(cfg):
    ...
    _write_text(cfg.pdy_file, content)
