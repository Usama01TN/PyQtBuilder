...
def preflight(args):
    ...
    # Detect and discard stale XML-format .pdt files from previous runs of
    # this builder before the TOML migration.  pyqtdeploy 3.x parses .pdt
    # as TOML; a leading "<?xml" or "<Project" makes the TOML loader bail
    # out on the first character with "invalid character in key name: '?'".
    if pdts:
        for stale in list(pdts):
            try:
                head = stale.read_text(encoding='utf-8', errors='ignore')[:200].lstrip()
            except OSError:
                head = ''
            if head.startswith('<?xml') or head.startswith('<Project'):
                log.info('  found stale XML .pdt at %s — deleting (pyqtdeploy 3.x wants TOML)', stale)
                stale.unlink()
                pdts.remove(stale)
...
def run_pyqtdeploy(args, sysroot_dir, venv_dir, qt_dir):
    ...
    pyqtdeploy 3.x parses .pdt files as TOML (see project/project.py:
    `_load_toml` calls `toml.load`).  We validate the file is TOML before
    invoking pyqtdeploy-build; otherwise pyqtdeploy gives a confusing
    error like:

        there was an error reading the project fil[e]:
        Found invalid character in key name: '?'.

    which is the TOML parser tripping on `<?xml` at the start of an
    XML-format .pdt (the format used by pyqtdeploy 2.x).  If we detect
...
def _validate_or_rewrite_pdt_as_toml(args):
    ...
    try:
        head = pdt.read_text(encoding='utf-8', errors='replace')
    except OSError as e:
